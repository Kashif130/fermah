...
CSV_FIELDS = ["handle", "title", "type", "emoji", "badge", "spotlight", "submitted_at"]
...
def load_submissions():
    ...
    try:
        with open(SUBMISSIONS_FILE, newline="", encoding="utf-8") as f:
            ...
            for row in reader:
                row["spotlight"] = row.get("spotlight", "False") == "True"
                rows.append(row)
    except Exception:
        return []
...
def save_submission(entry: dict):
    ...
    with open(SUBMISSIONS_FILE, "a", newline="", encoding="utf-8") as f:
        ...
        writer.writerow({
            ...
            "spotlight":    str(entry.get("spotlight", False)),
            "submitted_at": datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC"),
        })

def toggle_spotlight(handle: str, title: str):
    """Toggle spotlight status for a submission identified by handle+title."""
    if not os.path.exists(SUBMISSIONS_FILE):
        return
    rows = []
    with open(SUBMISSIONS_FILE, newline="", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            if row["handle"] == handle and row["title"] == title:
                row["spotlight"] = str(row["spotlight"] != "True")
            rows.append(row)
...
/* ── SPOTLIGHT ── */
.spotlight-section {
  background: linear-gradient(135deg, rgba(0,255,160,0.05), rgba(0,200,255,0.03));
  border: 1px solid var(--border-green);
...
.spotlight-header {
  ...
  text-shadow: 0 0 16px rgba(0,255,160,0.3);
}
.spotlight-card {
  background: rgba(0,255,160,0.04); border: 1px solid rgba(0,255,160,0.2);
  border-radius: 12px; padding: 14px 16px; margin-bottom: 10px;
  display: flex; align-items: center; gap: 12px;
  animation: glowPulse 3s ease-in-out infinite;
}
@keyframes glowPulse {
  0%,100% { box-shadow: 0 0 0px rgba(0,255,160,0); }
  50%      { box-shadow: 0 0 12px rgba(0,255,160,0.15); }
}
.spotlight-star { font-size: 20px; flex-shrink: 0; }
.spotlight-info { flex: 1; }
.spotlight-title { font-size: 14px; font-weight: 600; color: #fff; margin-bottom: 2px; }
.spotlight-meta  { font-family: 'Space Mono', monospace; font-size: 10px; color: var(--green); letter-spacing: .05em; }

/* feed item with spotlight glow */
.feed-item.spotlit {
...
with tab3:
    ...
    with left:
        ...
        if st.button("🚀 Submit to Creator Program"):
            if handle and title_inp and content_type:
                ...
                st.session_state.submissions = load_submissions()
                st.success("🚀 Submission saved! Good luck this week 🐬")
                st.rerun()
            else:
    ...
    # ── SPOTLIGHT SECTION ──
    spotlighted = [s for s in st.session_state.submissions if str(s.get("spotlight","")) == "True"]
    if spotlighted:
        st.markdown('<div class="spotlight-section">', unsafe_allow_html=True)
        st.markdown('<div class="spotlight-header">🌟 Spotlighted This Week</div>', unsafe_allow_html=True)
        for s in spotlighted:
            submitted_at = s.get("submitted_at", "")
            st.markdown(f"""
            <div class="spotlight-card">
              <div class="spotlight-star">{s['emoji']} ⭐</div>
              <div class="spotlight-info">
                <div class="spotlight-title">{s['title']}</div>
                <div class="spotlight-meta">{s['handle']} · {s['type'].upper()} · {submitted_at}</div>
              </div>
              <div class="feed-badge {s['badge']}">{s['type'].upper()}</div>
            </div>""", unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)

    # ── RECENT FEED with Spotlight buttons ──
    st.markdown('<div class="sec-title">📥 Recent Submissions Feed</div>', unsafe_allow_html=True)
    sample_feed = [
        {"handle":"@zkbuilder","title":"Fermah Froben Deep Dive Thread","emoji":"✍️","badge":"badge-thread","type":"thread","spotlight":False,"submitted_at":"Sample"},
        {"handle":"@cryptoart_pi","title":"Fermah x ZK — Motion Graphic","emoji":"🎨","badge":"badge-art","type":"art","spotlight":False,"submitted_at":"Sample"},
        {"handle":"@0xfermah_fan","title":"Fermah Explained in 60 Seconds","emoji":"🎥","badge":"badge-video","type":"video","spotlight":False,"submitted_at":"Sample"},
    ]
    all_feed = st.session_state.submissions + sample_feed
    for idx, e in enumerate(all_feed[:10]):
        is_spotlight = str(e.get("spotlight","")) == "True"
        spotlit_cls  = "spotlit" if is_spotlight else ""
        col_feed, col_btn = st.columns([5, 1])
        with col_feed:
            st.markdown(f"""
            <div class="feed-item {spotlit_cls}">
              <div class="feed-icon">{e['emoji']}{"⭐" if is_spotlight else ""}</div>
              <div style="flex:1">
                <div class="feed-title">{e['title']}</div>
                <div class="feed-meta">{e['handle']} · {e.get("submitted_at","Week 1")} · {e['type'].upper()}</div>
              </div>
              <div class="feed-badge {e['badge']}">{e['type'].upper()}</div>
            </div>""", unsafe_allow_html=True)
        with col_btn:
            # Only real submissions (from CSV) can be spotlighted
            is_real = e.get("submitted_at", "Sample") != "Sample"
            if is_real:
                btn_label = "★ Remove" if is_spotlight else "☆ Spotlight"
                btn_key   = f"spot_{e['handle']}_{e['title']}_{idx}"
                if st.button(btn_label, key=btn_key, use_container_width=True):
                    toggle_spotlight(e["handle"], e["title"])
                    st.session_state.submissions = load_submissions()
                    st.rerun()
